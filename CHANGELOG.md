...
### Changed
- Refreshed `README.md` to match the current codebase: `solve_ivp`-based Poppe, `estimate_temperatures` behavior, mass \(L/G\) from volumetric air flow, air/water API notes.

### Fixed
- `AirFlow.density` and `AirFlow.volume_to_dry_mass` now call `psychrolib.GetMoistAirDensity` with humidity ratio \(\omega\) (not relative humidity) and use the correct volume variable in the dry-air mass conversion.
